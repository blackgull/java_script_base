...
const snake = {
    ...
    makeStep() {
        ...
        this.body.pop();
        // Вызывем метод прохода змейки через стену
        this.throughWall();
    },

    /**
     * Проверяет границы стен и передаёт последущую точку движения змейки
     */
    throughWall() {
        const nextPoint = this.getNextStepHeadPoint();

        if (nextPoint.y < 0) {
            this.body.unshift({x: nextPoint.x, y: nextPoint.y + this.config.getRowsCount()});
            this.body.pop();
        }

        if (nextPoint.y > this.config.getRowsCount() - 1) {
            this.body.unshift({x: nextPoint.x, y: nextPoint.y - this.config.getRowsCount()});
            this.body.pop();
        }

        if (nextPoint.x < 0) {
            this.body.unshift({x: nextPoint.x + this.config.getColsCount(), y: nextPoint.y});
            this.body.pop();
        }

        if (nextPoint.x > this.config.getColsCount() - 1) {
            this.body.unshift({x: nextPoint.x - this.config.getColsCount(), y: nextPoint.y});
            this.body.pop();
        }
    },
    ...
    growUp() {
        ...
        // Вызываем метод для формирования и вывода счёта
        this.scorePoints();
    },

    /**
     * Формирует и выводит подсчёт очков
     */
    scorePoints() {
        // В массиве тела змейки вычитаем 1 элемент, полученную длину выводим в качестве счёта
